...
export abstract class CrudService<T> extends BaseService {
  ...
  create(model: T): Observable<T> {
    return this.httpClient.post<T>(this.url() + '/create', JSON.stringify(model), this.deductHeader());
  }

  update(model: T): Observable<T> {
    return this.httpClient.put<T>(this.url() + '/update', JSON.stringify(model), this.deductHeader());
  }
  ...
  remove(key: string): Observable<Message> {
    return this.httpClient.get<Message>(this.url() + '/delete/' + key, this.deductHeader());
  }
